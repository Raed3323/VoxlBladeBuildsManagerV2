...
const CACHE_KEY = 'voxlpedia-cache-v27'
...
const STAT_LABEL_ALIASES = {
  ...
  'Physical Scaling': 'physicalScaling',
  'Magic Scaling': 'magicScaling',
  'Holy Scaling': 'holyScaling',
  'Hex Scaling': 'hexScaling',
  'Fire Scaling': 'fireScaling',
  'Water Scaling': 'waterScaling',
  'Earth Scaling': 'earthScaling',
  'Air Scaling': 'airScaling',
  'Dexterity Scaling': 'dexterityScaling',
  'Summon Scaling': 'summonScaling',
}
...
const STAT_TYPES = {
  ...
  physicalScaling: 'multiplier', magicScaling: 'multiplier', holyScaling: 'multiplier',
  hexScaling: 'multiplier', fireScaling: 'multiplier', waterScaling: 'multiplier',
  earthScaling: 'multiplier', airScaling: 'multiplier', dexterityScaling: 'multiplier',
  summonScaling: 'multiplier',
}
...
function extractWikiStatsAndPerks(page) {
  ...
  const stats = {}
  const evidence = {}
  const tierMatch = corpus.match(/(?:^|\n|\|)\s*Tier\s*(?::|\| |=)\s*(\d+)\b/i) || corpus.match(/\bTier\s*(?::|\| |=)\s*(\d+)\b/i)
  ...
  const recordStat = (rawLabel, rawNum, hadPercent, sourceLine, forceStatId = null) => {
    const label = cleanText(rawLabel).replace(/\s+/g, ' ')
    const lower = label.toLowerCase()
    if (!label || META_WORDS.has(lower)) return false

    const statId = forceStatId || STAT_LABEL_TO_ID.get(lower)
  ...
  let scalingContext = false
  for (let i = 0; i < lines.length; i += 1) {
    const line = lines[i]
    const normalizedLine = normalize(line)
    if (/^damage scalings?$/.test(normalizedLine) || /damage scalings?/.test(normalizedLine)) {
      scalingContext = true
      // Some MediaWiki tabbers flatten the heading and its first values into
      // one line. Only skip the line when it contains no numeric stat fragment.
      if (!/[A-Za-z][A-Za-z0-9'’&()\- /]{1,80}\s*[:|]\s*[+-]?\d+(?:\.\d+)?/.test(line)) continue
    }
    if (/^(?:stat boosts?|perks?|effects?|description|obtain(?:ed)?|location|requirements?|upgrade|history|trivia|gallery)$/i.test(line.trim())) {
      scalingContext = false
    }
    ...
    if (scalingContext && !hadPercent) {
      const scalingMap = {
        physical: 'physicalScaling', magic: 'magicScaling', holy: 'holyScaling',
        hex: 'hexScaling', fire: 'fireScaling', water: 'waterScaling',
        earth: 'earthScaling', air: 'airScaling', dexterity: 'dexterityScaling',
        summon: 'summonScaling',
      }
      const scalingId = scalingMap[normalize(rawLabel)]
      if (scalingId && Number.isFinite(rawNum)) {
        recordStat(rawLabel + ' Scaling', rawNum, false, line, scalingId)
        continue
      }
    }
...
export async function resolveWikiItemStats(itemName, slotLabel, signal) {
  ...
  const info = smartNormalizeQuery(query, slotLabel, category)
  const cacheKey = `stats-resolve-v11:${category}:${normalize(slotLabel)}:${normalize(query)}`
  const cached = getCached(cacheKey)
