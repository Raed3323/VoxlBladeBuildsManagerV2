...
const SHRINE_BALANCE_MULTIPLIERS = Object.freeze({
  1: 3,
  2: 1.7,
  3: 1.4,
  4: 1.1,
...
const SHRINE_AFFECTED_CATEGORIES = new Set(['boosts', 'scalings'])

function isWeaponPartSlot(slot, item) {
  if (!slot || !item || slot.__fromRequirement) return false
  const slotLabel = String(slot.label || '').toLowerCase()
  const slotType = String(item.slotType || '').toLowerCase()
  return slotType.startsWith('weapon-') || /\b(handle|pole|blade|core|gloves|gauntlets)\b/i.test(slotLabel)
}

function shrineBalanceMultiplierForTier(tier) {
  const numericTier = Number(tier)
  return Number.isFinite(numericTier) ? (SHRINE_BALANCE_MULTIPLIERS[numericTier] ?? null) : null
}

function isShrineAffectedStat(statId) {
  const def = getStatDef(statId)
  return !!def && SHRINE_AFFECTED_CATEGORIES.has(def.category)
}
...
export function calculateBuildStats(build, wikiOverrides = {}, wikiEnchantmentOverrides = {}, options = {}) {
  const shrineOfBalanceEnabled = options?.shrineOfBalanceEnabled === true
  ...
  for (const originalSlot of loadout) {
    ...
    equippedSummary.push({
      ...
      tier: dbItem?.tier ?? dynamicItem?.tier ?? localItem?.tier ?? null,
      isWeaponPart: isWeaponPartSlot(slot, dbItem),
      shrineMultiplier: null,
      isInfusion,
    })
    ...
    const weaponPart = isWeaponPartSlot(slot, dbItem)
    const shrineMultiplier = shrineOfBalanceEnabled && weaponPart
      ? shrineBalanceMultiplierForTier(dbItem.tier)
      : null

    const summaryEntry = equippedSummary[equippedSummary.length - 1]
    if (summaryEntry) {
      summaryEntry.tier = dbItem.tier ?? null
      summaryEntry.isWeaponPart = weaponPart
      summaryEntry.shrineMultiplier = shrineMultiplier
    }

    if (shrineOfBalanceEnabled && weaponPart && shrineMultiplier == null) {
      warnings.push(`${slot.label}: Shrine of Balance is ON, but Wiki tier data for "${dbItem.name}" is unavailable. Shrine bonus was not guessed or applied.`)
    }
    ...
    for (const [statId, rawValue] of Object.entries(dbItem.stats || {})) {
      ...
      const shrineApplies = !isInfusion && weaponPart && shrineMultiplier != null && isShrineAffectedStat(statId)
      const value = base * statScale * (shrineApplies ? shrineMultiplier : 1)
      ...
      addSource(acc, statId, {
        label: isInfusion
          ...
          : shrineApplies
            ? `${dbItem.name} (${slot.label}) · Shrine of Balance ×${shrineMultiplier}`
      ...
      if (!isInfusion && forgePct > 0 && getStatDef(statId).id !== 'attackSpeed' && getStatDef(statId).category !== 'scalings') {
        const shrineAppliesToForge = weaponPart && shrineMultiplier != null && isShrineAffectedStat(statId)
        addSource(acc, statId, {
          label: shrineAppliesToForge
            ? `${dbItem.name} Forge +${forgeLevel} (+${forgePct}% of base) · Shrine ×${shrineMultiplier}`
            : `${dbItem.name} Forge +${forgeLevel} (+${forgePct}% of base)`,
          value: base * forgePct / 100 * (shrineAppliesToForge ? shrineMultiplier : 1),
          kind: 'forge',
          verified: dbItem.verified,
          slotId: slot.id,
          slotLabel: slot.label,
  ...
  for (const def of Object.values(STAT_DEFS)) {
    ...
    const precision = def.type === 'multiplier' ? 100 : 10
    const total = Math.round(data.total * precision) / precision
  ...
  return {
    ...
    shrineOfBalance: {
      ...
      multipliers: SHRINE_BALANCE_MULTIPLIERS,
    },
  }
}
